...
class Install
{
    ...
    public static function install() {
		...
		if ( 'yes' === get_transient( 'dw_nlite_installing' ) ) {
			return;
        }

		// If we made it till here nothing is running yet, lets set the transient now.
		set_transient( 'dw_nlite_installing', 'yes', MINUTE_IN_SECONDS * 10 );
		self::create_tables();
		self::setup_stuffs();
		self::create_options();
		self::create_roles();
		...
		flush_rewrite_rules();
		delete_transient( 'dw_nlite_installing' );
        do_action('dw_nlite_instlalled');
    }
    ...
    private static function get_schema() {
        ...
        $tables = "
CREATE TABLE {$wpdb->prefix}dw_nlite(
	...
	public static function get_tables() {
		...
		$tables = apply_filters( 'dw_nlite_install_get_tables', $tables );
		return $tables;
    }
